...
def calculate_propeller_performance(R, Vs, t, w, etaR, zP, Z, D, PdD, AEdAO, hk, Ta):
    # Water input data
    ...
    # Reynolds number
    Rn = np.sqrt(Va ** 2 + (0.75 * np.pi * D * n) ** 2) * C075R / nu  # Reynolds number
    ...
    if Rn > 2e6:
        ...
        while eRn > 0.0001:
            deltaKt = 0.000353485 \
                    ...
                    + 0.000257792 * (np.log10(Rn) - 0.301)**2 * AEdAO * J**2 \
                    + 0.0000643192 * (np.log10(Rn) - 0.301) * PdD**6 * J**2 \
                    - 0.0000110636 * (np.log10(Rn) - 0.301)**2 * PdD**6 * J**2 \
                    - 0.0000276305 * (np.log10(Rn) - 0.301)**2 * Z * AEdAO * J**2 \
                    + 0.0000954 * (np.log10(Rn) - 0.301) * Z * AEdAO * PdD * J \
                    + 0.0000032049 * (np.log10(Rn) - 0.301) * Z**2 * AEdAO * PdD**3 * J

            deltaKq = -0.000591412 \
                    + 0.00696898 * PdD \
                    - 0.0000666654 * Z * PdD**6 \
                    + 0.0160818 * AEdAO**2 \
                    - 0.000938091 * (np.log10(Rn) - 0.301) * PdD \
                    - 0.00059593 * (np.log10(Rn) - 0.301) * PdD**2 \
                    + 0.0000782099 * (np.log10(Rn) - 0.301)**2 * PdD**2 \
                    + 0.0000052199 * (np.log10(Rn) - 0.301) * Z * AEdAO * J**2 \
                    - 0.00000088528 * (np.log10(Rn) - 0.301)**2 * Z * AEdAO * PdD * J \
                    + 0.0000230171 * (np.log10(Rn) - 0.301) * Z * PdD**6 \
                    - 0.00000184341 * (np.log10(Rn) - 0.301)**2 * Z * PdD**6 \
                    - 0.00400252 * (np.log10(Rn) - 0.301) * AEdAO**2 \
                    + 0.000220915 * (np.log10(Rn) - 0.301)**2 * AEdAO**2
    ...
    if n > 900 or n < 855:
        with open('constraints.txt', 'a') as fID:
            fID.write(f'{Vs * 3600 / 1852} {D} {Z} {AEdAO} {PdD} {zP} ShaftSpeed\n')
    ...
    if t075dD < tmin075dD:
        with open('constraints.txt', 'a') as fID:
            fID.write(f'{Vs * 3600 / 1852} {D} {Z} {AEdAO} {PdD} {zP} Strength\n')
        raise ValueError('Strength constraint has been reached!')
    """
    ...
    sigma = np.array([0.096862500000000004,
                        0.10587000000000001,
                        0.106216,
                        0.109273,
                        0.119423,
    ...
    tal5 = np.array([0.040322299999999998, 
    ...
    cavLim = interp1d(sigma, tal5, kind='linear', fill_value='extrapolate')(sig07R)
    cavLim = float(cavLim)

    """
    if tal07R > cavLim:
        with open('constraints.txt', 'a') as fID:
            fID.write(f'{Vs * 3600 / 1852} {D} {Z} {AEdAO} {PdD} {zP} Cavitation\n')
        raise ValueError('Cavitation constraint has been reached!')
    """
    ...
    if Vtip > Vtipmax:
        with open('constraints.txt', 'a') as fID:
            fID.write(f'{Vs * 3600 / 1852} {D} {Z} {AEdAO} {PdD} {zP} Velocity\n')
        raise ValueError('Peripherical velocity constraint has been reached!')
    """
