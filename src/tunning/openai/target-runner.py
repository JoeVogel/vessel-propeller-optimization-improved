...
if __name__ == '__main__':
    ...
    while cand_params:
        ...
        if param == "--sigma_init":
            sigma_init = float(value)
        elif param == "--sigma_decay":
            sigma_decay = float(value)
        elif param == "--learning_rate":
            learning_rate = float(value)
        elif param == "--learning_rate_decay":
            learning_rate_decay = float(value)
    ...
    # Sanity checks
    if None in [sigma_init, sigma_decay, learning_rate, learning_rate_decay, weight_decay, num_population]:
        target_runner_error("Missing parameter value!")

    # Run and print the output
    try:
        fitness = run(instance, sigma_init, sigma_decay, learning_rate, learning_rate_decay, weight_decay, num_population)
        print(fitness)
        sys.exit(0)
